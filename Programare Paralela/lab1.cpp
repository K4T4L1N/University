...
//rankul de proces si numarul acestora
int rank, processes_amount;

const int COMPOSE_MANUALLY = 1;
const int ROOT_RANK = 0;

//intruducerea matricii de la tastatura (keyboard matrix input algorithm)
...
void compose_matrix(int*& matrix, int rows, int cols) {
	matrix = new int[cols * rows];
	for (int i = 0; i < rows; i++) {
		matrix[i * cols] = i; //stabilim o variabila de contor (ca procesele sa stie cu ce rand lucreaza
		for (int j = 1; j < cols; j++) {
...
//algoritmul de compunere a inversei matricii
...
//concatinarea indecilor maximali
...
//algoritmul de iterare pe linie de matrice pentru gasirea elementului maximal in linie
void find_max_in_line(int* v_in, int* v_mins, int len, int rows, int cols) {
	int* in = v_in;
	int* row_max_indexes = new int[cols + 1];
	int size = 1;
	int max = in[1];
	row_max_indexes[0] = in[0]; //row index
...
//golim buferul de valori/ indexul
	delete[]row_max_indexes;
}
//gasirea valorilor de echilibru (maximale)
...
int main(int argc, char* argv[])
{
	...
	if (rank == ROOT_RANK) {
		...
		cin >> cols;
		cols++; //value to be incremented to store row indexes
	}
...
//definim (prin input) valorile de input
		cout << "\nMATRIX A";
		show_matrix(matrix_A, rows, cols);
		cout << "\nMATRIX B";
		show_matrix(matrix_B, rows, cols);

//compunem inversa matricii
		make_inverse(matrix_A, rows, cols, inverse_A);
	}

//a barier, to wait the processes to finish their work with matrix values
	MPI_Barrier(MPI_COMM_WORLD);
...
//gasirea valorilor de intersectie sau echilibru, sau cum acolo
	if (rank == ROOT_RANK) {
		cout << "Intersections : ";
		for (int i = 0; i < rows; i++) {
