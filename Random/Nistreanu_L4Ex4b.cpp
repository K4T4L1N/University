...
//Procesul cu rankul root aloca spatiul si initializeaza matrice
	if(myrank==root)                                                
	{                                                                // daca rankul este egal cu numarul procesului radacina
...
MPI_Scatter(Matr_Init, sendcount, MPI_DOUBLE,Rows, reccount, MPI_DOUBLE, root, MPI_COMM_WORLD);                    // distribuirea colectiva a datelor 
for(int i=1; i<numtask; i++){
        for(int j=0; j<numtask; j++){
                if(myrank==root){                                                                 // daca rankul este egal cu nr procesului radacina se transmite mesajul
                MPI_Send(&Matr_Init[i*numtask+j], 1, MPI_DOUBLE, i, 10, MPI_COMM_WORLD);
            }
                if(myrank==i){                                                                // daca rankul este egal cu i se receptioneaza mesajele in vectorul Rows
...
for(int j=0; j<numtask; j++){                             // se parcurge doar un ciclu
    if(myrank==root){
        MPI_Sendrecv(&Matr_Init[j], 1, MPI_DOUBLE, root+1, 10,&Rows[j], 1, MPI_DOUBLE, root+1, 10, MPI_COMM_WORLD, &status);             // daca rankul este egal cu nr procesului radacina prin sendrecv se transmite mesajul destinatarului + 1
    }
    if(myrank==root+1){
	        MPI_Sendrecv(&Rows2[j], 1, MPI_DOUBLE, root, 10,&Rows2[j], 1, MPI_DOUBLE, root, 10, MPI_COMM_WORLD, &status);           // daca rankul este egal cu nr procesului radacina + 1 prin sendrecv se transmite mesajul destinatarului     }
...
printf("\n");
printf("Resultatele f-tiei Sendrecv pentru procesul cu rankul %d \n", myrank);
for (i=0; i<numtask; ++i)
